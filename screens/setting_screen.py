...
class SettingsScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        root = Background(orientation="vertical", padding=20, spacing=20)
        ...
        # Контейнер настроек
        box = BoxLayout(orientation="vertical", spacing=20, size_hint_y=0.6)
        ...
        self.goal_slider = Slider(
            min=GOAL_SLIDER["min"], max=GOAL_SLIDER["max"], value=GOAL_SLIDER["default"]
        )
        self.goal_slider.bind(value=self.on_goal_change)

        self.goal_value = Label(
            text=f"{GOAL_SLIDER['default']} мин", color=COLORS["accent"]
        )
        ...
        btns.add_widget(Button(text="Сохранить", background_color=COLORS["success"]))
        btns.add_widget(Button(text="Сбросить", background_color=COLORS["danger"]))
