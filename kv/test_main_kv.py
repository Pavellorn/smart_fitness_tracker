...
from kivy.uix.screenmanager import ScreenManager, Screen, SlideTransition  # Для управления экранами и переходами между ними
from kivy.uix.boxlayout import BoxLayout      # Контейнер, который располагает дочерние элементы в ряд или колонку
from kivy.uix.gridlayout import GridLayout    # Контейнер-сетка для расположения элементов в таблицу
...
from kivy.graphics import Color, Ellipse, Rectangle  # Для рисования графических примитивов
...
class AnimatedCircle(Button):
    ...
    def __init__(self, **kwargs):
        ...
        self.background_color = (0, 0, 0, 0)  # R,G,B,A - все нули (полностью прозрачный)
        ...
        anim_up = Animation(font_size=36, duration=0.3)  # Увеличиваем до 36 за 0.3 секунды
        # Анимация уменьшения размера шрифта
        anim_down = Animation(font_size=28, duration=0.3)  # Уменьшаем до 28 за 0.3 секунды
...
class TrainingScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        root.add_widget(Label(text='ТРЕНИРОВКА', font_size=32,
        ...
        buttons = BoxLayout(spacing=10, size_hint_y=0.2)  # Занимает 20% высоты, расстояние 10px между кнопками
        
        # Добавляем три кнопки с разным временем и цветами
        buttons.add_widget(Button(text='30 мин', background_color=(0.2, 0.8, 0.2, 1)))  # Зеленая
        buttons.add_widget(Button(text='60 мин', background_color=(1, 0.5, 0, 1)))      # Оранжевая
        buttons.add_widget(Button(text='90 мин', background_color=(0.8, 0.2, 0.2, 1)))  # Красная
...
class StatsScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        root.add_widget(Label(text='📊 ЗА НЕДЕЛЮ', font_size=28,
                              color=(1, 1, 1, 1), bold=True, size_hint_y=0.1))
        
        # Отображение общего времени тренировок за неделю
        root.add_widget(Label(text='12ч 30мин 🏆', font_size=32,
                              color=(1, 0.9, 0.2, 1), size_hint_y=0.1))

        # Создаем таблицу для отображения статистики по дням недели
        days = GridLayout(cols=7, size_hint_y=0.3)  # 7 колонок (по числу дней), 30% высоты
        ...
        root.add_widget(Label(text='Лучший день: Пятница 20ч',
                              font_size=16, color=(0.2, 0.8, 0.2, 1), size_hint_y=0.1))

        # Кнопка возврата на главный экран
        back = Button(text='← Назад', size_hint=(0.4, 0.1),  # 40% ширины, 10% высоты
...
class SettingsScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        root.add_widget(Label(text='НАСТРОЙКИ', font_size=32,
                              color=(1, 1, 1, 1), bold=True, size_hint_y=0.1))

        # Основной контейнер для элементов настроек
        box = BoxLayout(orientation='vertical', spacing=20, size_hint_y=0.6)  # 60% высоты
        ...
        btns.add_widget(Button(text='Сохранить', background_color=(0.2, 0.6, 0.2, 1)))  # Зеленая
        btns.add_widget(Button(text='Сбросить', background_color=(0.8, 0.2, 0.2, 1)))   # Красная
        root.add_widget(btns)

        # ----- КНОПКА ВОЗВРАТА -----
        back = Button(text='← Назад', size_hint=(0.4, 0.1),  # 40% ширины, 10% высоты
                      pos_hint={'center_x': 0.5})  # Центрирование
        # Привязываем событие нажатия к смене экрана на главный
        back.bind(on_press=lambda *_: setattr(self.manager, 'current', 'main'))
        root.add_widget(back)
...
class UIApp(App):
    ...
    def build(self):
        ...
        sm = ScreenManager(transition=SlideTransition(duration=0.3))  # длительность 0.3 секунды
